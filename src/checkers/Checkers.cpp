...
{
	Checkers* checkers = (Checkers*)a_clientData;

	Renderer* renderer = checkers->GetRenderer();

	float maxHeight;

	//I do not need to clean up the physics heightmap stuff here now, because the function to generate a new one now automatically removes the old one.
...
int Checkers::Init()
{
	...
	m_renderer->LoadAmbient("../data/tablecloth.jpg", m_clothModels[0]);
	m_renderer->LoadTexture("../data/tablecloth.jpg", m_clothModels[0]);
	m_renderer->LoadSpecularMap("../data/tablecloth.jpg", m_clothModels[0]);
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		m_renderer->LoadTexture("", m_models[m_models.size() - 1]);
		m_renderer->LoadAmbient("", m_models[m_models.size() - 1]);
	...
	TwAddButton(m_debugBar, "Re-generate", BoardGenerate, (void*)this, "");
...
void Checkers::Update(float a_deltaTime)
{
...
float Checkers::GetHeightAtPos(float a_x, float a_z)
{
	PxRaycastBuffer hit;
...
void Checkers::ResetCheckerPieceHeights()
{
	for (unsigned int i = 0; i < 8; ++i)
	{
		for (unsigned int j = 0; j < 8; ++j)
		{
			if (m_board[i][j] != -1)
			{
				...
				m_renderer->SetLightPosition(m_pieceLights[m_board[i][j]], vec3(pos.x, 5.0f + GetHeightAtPos(pos.x, pos.z), pos.z));
			}
		}
	}
}
...
void Checkers::CheckersUpdate(float a_deltaTime)
{
...
#pragma endregion
	...
	if (m_inputTimer < 0)
	{
		...
		{
			std::thread thr(&Checkers::UseAIMove, this);
...
void Checkers::HandleEnter(int(&a_board)[8][8], const unsigned int a_xPos, const unsigned int a_yPos, unsigned int &a_prevX, unsigned int &a_prevY, bool &a_turn, const bool a_changeEmitters, unsigned int& a_pieceSelected)
{
					...
					for (unsigned int i = 0; i < 8; ++i)
					{
						for (unsigned int j = 0; j < 8; ++j)
						{
							...
							else
							{
								...
								{
									//Check each direction for opponent pieces
									if ((i < 6 && j > 1 && a_board[i + 1][j - 1] < 12 && a_board[i + 1][j - 1] != -1 && a_board[i + 2][j - 2] == -1) ||
...
bool Checkers::ValidMove(const int a_board[8][8], const unsigned int a_xPos, const unsigned int a_yPos, const unsigned int a_prevX, const unsigned int a_prevY, const bool a_turn, bool a_changeEmitters)
{
	//Capture move
...
void Checkers::AIMove(int(&a_board)[8][8], const bool a_turn, const unsigned int a_difficulty)
{
...
std::vector<std::vector<std::vector<int>>> Checkers::GetPossibleMoves(const int a_board[8][8], const bool a_turn, std::vector<std::tuple<unsigned int, unsigned int>>& a_pieceToMove)
{
...
int Checkers::PlayUntilEnd(std::vector<std::vector<int>> a_board, const bool a_turn)
{
	int board[8][8];
	for (int i = 0; i < 8; ++i)
	{
